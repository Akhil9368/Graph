    ...
    private static void mst(int[][] graph) {
        ...
        for (int i = 1; i < TOTAL_VERTEX; i++) {
            ...
            for (int j = 1; j < TOTAL_VERTEX; j++) {
                if(graph[SourceVertex][j] != 0 && graph[SourceVertex][j]!=Integer.MAX_VALUE &&
                        !Mset[j] &&(graph[SourceVertex][j] + weight[SourceVertex])<weight[j]){
                    weight[j] =graph[SourceVertex][j] + weight[SourceVertex];
                    parent[j]= SourceVertex;
                }
            }
        }
        for (int i=1;i<TOTAL_VERTEX;i++){
            System.out.println(parent[i] + " ==> " + i + " " + " Cost is " + graph[parent[i]][i]);
        }
    }
